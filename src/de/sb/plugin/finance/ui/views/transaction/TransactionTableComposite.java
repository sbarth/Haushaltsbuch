...
public class TransactionTableComposite implements PropertyChangeListener {
	...
	private final int[] tableColumnAlignment = R.TABLE_TRANSACTION_COLUMN_ALIGNMENT;
	...
	public TransactionTableComposite(final Composite parent, final IWorkbenchPartSite workbenchSite, final TableTransactionFilter filter) {
		...
		if (tableColumnNames.length != tableColumnWidths.length || tableColumnNames.length != tableColumnAlignment.length) {
	...
	private void addTableTreeViewer() {
		...
		for (int index = 0; index < tableColumnNames.length; index++) {
			int columnAlignment = tableColumnAlignment[index];
			int columnWidth = tableColumnWidths[index];
			String columnName = tableColumnNames[index];

			createTreeViewerColumn(columnName, columnWidth, columnAlignment);
		}
	...
	private void createTreeViewerColumn(final String columnName, final int columnWidth, final int alignment) {
		...
		if (columnName.equals(R.COLUMN_NAME_AMOUNT)) {
			tvCol.getColumn().setAlignment(SWT.RIGHT);
		}
	...
	private void setInput(final TreeNode treeNode) {
